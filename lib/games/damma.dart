...
class _DamaHomePageState extends State<DamaHomePage> {
  ...
  void handleTap(int row, int col) {
    ...
    final selectedPiece = (selectedRow != null && selectedCol != null)
  ...
  bool isValidMove(int fromRow, int fromCol, int toRow, int toCol) {
    ...
    if ((rowDiff.abs() == 1) && (colDiff.abs() == 1)) {
      if (!isKing(piece)) {
        if (isWhitePiece && rowDiff != -1) return false;
        if (isBlackPiece && rowDiff != 1) return false;
      }
      return true;
    }
    ...
    if ((rowDiff.abs() == 2) && (colDiff.abs() == 2)) {
      ...
      if (middle == PieceType.none) return false;
      if (isWhitePiece &&
